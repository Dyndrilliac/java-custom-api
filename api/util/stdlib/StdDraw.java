...
/*************************************************************************
 * Compilation: javac StdDraw.java Execution: java StdDraw Standard drawing library. This class provides a basic capability for creating drawings with
 * your programs. It uses a simple graphics model that allows you to create drawings consisting of points, lines, and curves in a window on your
 * computer and to save the drawings to a file. Todo ---- - Add support for gradient fill, etc. - Fix setCanvasSize() so that it can only be called
 * once. - On some systems, drawing a line (or other shape) that extends way beyond canvas (e.g., to infinity) dimensions does not get drawn. Remarks
 * ------- - don't use AffineTransform for rescaling since it inverts images and strings - careful using setFont in inner loop within an animation -
...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener
{
    ...
    private static void init()
    {
        ...
        // frame stuff
        ImageIcon icon = new ImageIcon(StdDraw.onscreenImage);
     ...
     * keys (such as F1 and arrow keys) and modifier keys (such as shift and control). See <a href =
    ...
    {
        Image image = StdDraw.getImage(s);
        double xs = StdDraw.scaleX(x);
        double ys = StdDraw.scaleY(y);
    ...
    public static void picture(final double x, final double y, final String s, final double w, final double h)
    {
        ...
        double ws = StdDraw.factorX(w);
        double hs = StdDraw.factorY(h);
        if ((ws < 0) || (hs < 0))
        {
            throw new IllegalArgumentException("image " + s + " is corrupt");
        }
        ...
        {
            StdDraw.offscreen.drawImage(image,
    ...
    public static void point(final double x, final double y)
    {
        ...
        {
            StdDraw.offscreen.fill(new Ellipse2D.Double(xs - (scaledPenRadius / 2), ys - (scaledPenRadius / 2), scaledPenRadius, scaledPenRadius));
        }
        StdDraw.draw();
    }
    ...
    public static void save(final String filename)
    {
        ...
        else if (suffix.toLowerCase().equals("jpg"))
        {
            ...
            newRaster = raster.createWritableChild(0, 0, StdDraw.width, StdDraw.height, 0, 0, new int[] {
                0, 1, 2
            });
