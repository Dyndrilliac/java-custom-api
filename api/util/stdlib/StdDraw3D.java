...
/******************************************************************************
 * StdDraw3D.java Hayk Martirosyan 3D Drawing Library Standard Draw 3D is a Java library with the express goal of making it simple to create
 * three-dimensional models, simulations, and games. Introductory Tutorial: http://introcs.cs.princeton.edu/java/stddraw3d Reference manual:
 * http://introcs.cs.princeton.edu/java/stddraw3d-manual.html NOTE: The code below is only partially documented. Refer to the reference manual for
...
/**
 * Standard Draw 3D is a Java library with the express goal of making it simple to create three-dimensional models, simulations, and games. Here is a
 * <a href = "http://introcs.cs.princeton.edu/java/stddraw3d">StdDraw3D tutorial</a> and the <a href =
 * "http://introcs.cs.princeton.edu/java/stddraw3d/stddraw3d-manual.html">StdDraw3D reference manual</a>.
 *
...
WindowFocusListener
{
    ...
    /**
     * The camera can be controlled with the static functions already listed in this reference. However, much more advanced control of the camera can
     * be obtained by manipulating the Camera object. The Camera is basically equivalent to a Shape with a couple of extra functions, so it can be
     * moved and rotated just like a Shape. This functionality works well with point-of-view simulations and games.
     */
    ...
    /**
     * When you create a light in StdDraw3D, it returns a Light object. Light objects can be manipulated just like Shapes, and are useful if you want
    ...
    /**
     * Everything three-dimensional you draw in your scene from spheres to points to 3D text is actually a Shape object. When you call a drawing
     * function, it always returns a Shape object. If you keep a reference to this object, you can manipulate the already drawn Shape instead of
     * clearing and redrawing it, which is a much more powerful and more efficient method of animation.
     */
    ...
    public static class Vector3D
    {
        ...
        // --------------------------------------------------------------------------
        /*
         * public Vector3D (double min, double max) { this.x = min + Math.random() * (max - min); this.y = min + Math.random() * (max - min); this.z =
         ...
         */
        /*
         * public void drawLine (Vector3D origin) { Vector3D end = this.plus(origin); StdDraw3D.line(origin.x, origin.y, origin.z, end.x, end.y,
         * end.z); }
         */
    }
     ...
     */
    public static Shape box(final double x,
        final double y,
        final double z,
        final double w,
        final double h,
        ...
        final String imageURL)
    {
        ...
        com.sun.j3d.utils.geometry.Box box = new com.sun.j3d.utils.geometry.Box(dimensions.x,
            dimensions.y,
            dimensions.z,
            StdDraw3D.PRIMFLAGS,
            ap,
            StdDraw3D.numDivisions);
     ...
     */
    public static Shape cone(final double x,
        final double y,
        final double z,
        final double r,
        final double h,
    ...
    /**
     * Returns an identical copy of a Shape that can be controlled independently. Much more efficient than redrawing a specific shape or model.
     */
    ...
    private static JMenuBar createMenuBar()
    {
        ...
        StdDraw3D.loadButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_L, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        ...
        StdDraw3D.saveButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        ...
        StdDraw3D.save3DButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        ...
        StdDraw3D.quitButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Q, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        ...
        StdDraw3D.orbitModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_1, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        ...
        StdDraw3D.fpsModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_2, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        ...
        StdDraw3D.airplaneModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_3, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        ...
        StdDraw3D.lookModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_4, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        ...
        StdDraw3D.fixedModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_5, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        ...
        SpinnerNumberModel snm2 = new SpinnerNumberModel(StdDraw3D.DEFAULT_NUM_DIVISIONS, 4, 4000, 5);
     ...
     */
    public static Shape cube(final double x,
        final double y,
        final double z,
        final double r,
        final double xA,
     ...
     */
    public static Shape cylinder(final double x,
        final double y,
        final double z,
        final double r,
        final double h,
     ...
     */
    public static Shape ellipsoid(final double x,
        final double y,
        final double z,
        final double w,
        final double h,
    ...
    /**
     * Allows for camera navigation of a scene without redrawing. Useful when drawing a complicated scene once and then exploring without redrawing.
    ...
    private static void infoDisplay()
    {
        ...
        else if (StdDraw3D.cameraMode == StdDraw3D.FPS_MODE)
        {
            mode = "Camera: FPS_MODE";
        }
        else if (StdDraw3D.cameraMode == StdDraw3D.AIRPLANE_MODE)
        {
            mode = "Camera: AIRPLANE_MODE";
        }
        else if (StdDraw3D.cameraMode == StdDraw3D.LOOK_MODE)
        {
            mode = "Camera: LOOK_MODE";
        }
        else if (StdDraw3D.cameraMode == StdDraw3D.FIXED_MODE)
        {
        ...
        double d = b / 4;
        g.draw(new Line2D.Double(StdDraw3D.scaleX(d + center),
            StdDraw3D.scaleY(0 + center),
            StdDraw3D.scaleX(-d + center),
            StdDraw3D.scaleY(0 + center)));
        g.draw(new Line2D.Double(StdDraw3D.scaleX(0 + center),
            StdDraw3D.scaleY(d + center),
            StdDraw3D.scaleX(0 + center),
            StdDraw3D.scaleY(-d + center)));
    ...
    private static void initializeCanvas()
    {
        ...
        StdDraw3D.canvas = new Canvas3D(config)
        {
            ...
            /*
             * public void postSwap () { while (!showedOnce) { try { Thread.currentThread().sleep(5); } catch (InterruptedException e) {
             * System.out.println("Error sleeping"); } } //if (!showedOnce) { // try { Thread.currentThread().sleep(30); } // catch
             * (InterruptedException e) { System.out.println("Error sleeping"); } //} //StdOut.println("showed once is false, rendered once");
    ...
    /**
     * Is the given key currently pressed down? The keys correnspond to physical keys, not characters. For letters, use the uppercase character to
     * refer to the key. For arrow keys and modifiers such as shift and ctrl, refer to the KeyEvent constants, such as KeyEvent.VK_SHIFT.
     *
    ...
    /**
     * Draws a set of connected lines. For example, the first line is from (x[0], y[0], z[0]) to (x[1], y[1], z[1]), and the second line is from
     * (x[1], y[1], z[1]) to (x[2], y[2], z[2]). Much more efficient than drawing individual lines.
     */
    public static Shape lines(final double[] x, final double[] y, final double[] z)
    {
        ...
        GeometryArray geom = new LineStripArray(coords.length, GeometryArray.COORDINATES, new int[] {
            coords.length
        });
    ...
    /**
     * Draws a set of connected lines. For example, the first line is from (x[0], y[0], z[0]) to (x[1], y[1], z[1]), and the second line is from
     * (x[1], y[1], z[1]) to (x[2], y[2], z[2]). Much more efficient than drawing individual lines. Vertex colors are specified by the given array,
    ...
    public static Shape lines(final double[] x, final double[] y, final double[] z, final Color[] colors)
    {
        ...
        GeometryArray geom = new LineStripArray(coords.length, GeometryArray.COORDINATES | GeometryArray.COLOR_4, new int[] {
            coords.length
        });
        geom.setCoordinates(0, coords);
    ...
    private static void mouseMotionEvents(final MouseEvent e, final double newX, final double newY, final boolean dragged)
    {
        ...
        if ((StdDraw3D.cameraMode == StdDraw3D.ORBIT_MODE) && (dragged && StdDraw3D.isKeyPressed(KeyEvent.VK_ALT))
    ...
    public static void overlayArc(final double x, final double y, final double r, final double angle1, double angle2)
    {
        ...
        {
            StdDraw3D.getGraphics2D(StdDraw3D.offscreenImage).draw(new Arc2D.Double(xs - (ws / 2),
                ys - (hs / 2),
                ws,
                hs,
                angle1,
    ...
    {
        StdDraw3D.getGraphics2D(StdDraw3D.offscreenImage).draw(new Line2D.Double(StdDraw3D.scaleX(x0),
            StdDraw3D.scaleY(y0),
            StdDraw3D.scaleX(x1),
            StdDraw3D.scaleY(y1)));
    }
    ...
    public static void overlayPicture(final double x, final double y, final String s, final double w, final double h)
    {
        ...
        {
            StdDraw3D.getGraphics2D(StdDraw3D.offscreenImage).drawImage(image,
    ...
    public static void overlayPoint(final double x, final double y)
    {
        ...
        {
            StdDraw3D.getGraphics2D(StdDraw3D.offscreenImage).fill(new Ellipse2D.Double(StdDraw3D.scaleX(x) - (r / 2),
                StdDraw3D.scaleY(y) - (r / 2),
                r,
                r));
        }
    }
    ...
    /**
     * Draws a set of points at the given coordinates. For example, the first point is at (x[0], y[0], z[0]). Much more efficient than drawing
     * individual points.
     */
    ...
    /**
     * Draws a set of points at the given coordinates with the given colors. For example, the first point is at (x[0], y[0], z[0]) with color
     * colors[0]. Much more efficient than drawing individual points.
     */
    ...
    private static Shape polygon(final double[] x, final double[] y, final double[] z, final boolean filled)
    {
        ...
        Point3f[] coords = StdDraw3D.constructPoint3f(x, y, z);
        GeometryArray geom = new TriangleFanArray(coords.length, GeometryArray.COORDINATES, new int[] {
            coords.length
        });
        geom.setCoordinates(0, coords);
    ...
    primitive(final Primitive shape, final double x, final double y, final double z, final Vector3d angles, final Vector3d scales)
    {
    ...
    public static void save(final String filename)
    {
        ...
        else if (extension.equals("jpg"))
        {
            ...
            newRaster = raster.createWritableChild(0, 0, StdDraw3D.width, StdDraw3D.height, 0, 0, new int[] {
                0, 1, 2
            });
    ...
    /**
     * Sets anti-aliasing on or off. Anti-aliasing makes graphics look much smoother, but is very resource heavy. It is good for saving images that
    ...
    public static void setBackgroundSphere(final String imageURL)
    {
        ...
        Sphere sphere = new Sphere(1.1f,
            Primitive.GENERATE_NORMALS | Primitive.GENERATE_NORMALS_INWARD | Primitive.GENERATE_TEXTURE_COORDS,
            StdDraw3D.numDivisions);
    ...
    /**
     * Sets the number of triangular divisons of curved objects. The default is 100 divisons. Decrease this to increase performance.
     *
    ...
    /**
     * Sets the projection mode to orthographic projection. In this mode, parallel lines remain parallel after projection, and there is no
     * perspective. It is as looking from infinitely far away with a telescope. AutoCAD programs use this projection mode.
     */
    ...
    /**
     * Sets the projection mode to perspective projection with the given field of view. In this mode, closer objects appear larger, as in real life. A
     * larger field of view means that there is more perspective distortion. Reasonable values are between 0.5 and 3.0.
     *
     ...
     */
    public static Shape sphere(final double x,
        final double y,
        final double z,
        final double r,
        final double xA,
    ...
    /**
     * Draws a 3D text object at (x, y, z) with Euler rotation angles (xA, yA, zA). Uses the pen font, color, and transparency.
     */
    ...
    /**
     * Draws triangles which are defined by x1=points[i][0], y1=points[i][1], z1=points[i][2], x2=points[i][3], etc. All of the points will be the
     * width and color specified by the setPenColor and setPenWidth methods.
     *
     * @param points an array of the points to be connected. The first dimension is unspecified, but the second dimension should be 9 (the 3-space
    ...
    /**
     * Draws a set of triangles, each with the specified color. There should be one color for each triangle. This is much more efficient than drawing
    ...
    /**
     * Draws a series of cylindrical tubes of radius r, with vertices (x, y, z) and the given colors. No more efficient than drawing individual tubes.
     */
     ...
     */
    public static Shape wireBox(final double x,
        final double y,
        final double z,
        final double w,
        final double h,
     ...
     */
    public static Shape wireCone(final double x,
        final double y,
        final double z,
        final double r,
        final double h,
     ...
     */
    public static Shape wireCylinder(final double x,
        final double y,
        final double z,
        final double r,
        final double h,
     ...
     */
    public static Shape wireEllipsoid(final double x,
        final double y,
        final double z,
        final double w,
        final double h,
    ...
    /**
     * Draws a set of wireframetriangles, each with the specified color. There should be one color for each triangle. This is much more efficient than
