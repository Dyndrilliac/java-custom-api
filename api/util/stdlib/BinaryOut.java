...
/*************************************************************************
 * Compilation: javac BinaryOut.java Execution: java BinaryOut Write binary data to an output stream, either one 1-bit boolean, one 8-bit char, one
 * 32-bit int, one 64-bit double, one 32-bit float, or one 64-bit long at a time. The output stream can be standard output, a file, an OutputStream or
 * a Socket. The bytes written are not aligned. [wayne 7.17.2013] fixed bugs in write(char x, int r) and write(int x, int r) to add return statement
 * for (r == 8) and (r == 32) cases, respectively.
 *************************************************************************/
...
/**
 * <i>Binary output</i>. This class provides methods for converting primtive type variables (<tt>boolean</tt>, <tt>byte</tt>, <tt>char</tt>,
 * <tt>int</tt>, <tt>long</tt>, <tt>float</tt>, and <tt>double</tt>) to sequences of bits and writing them to an output stream. The output stream can
 * be standard output, a file, an OutputStream or a Socket. Uses big-endian (most-significant byte first).
 * <p>
 * The client must <tt>flush()</tt> the output stream when finished writing bits.
 * <p>
 * The client should not intermixing calls to <tt>BinaryOut</tt> with calls to <tt>Out</tt>; otherwise unexpected behavior will result.
 *
...
public final class BinaryOut
{
     ...
     */
    public BinaryOut(final String s)
    {
        try
        {
    ...
    {
        this.writeByte((int)((x >>> 56) & 0xff));
        this.writeByte((int)((x >>> 48) & 0xff));
        this.writeByte((int)((x >>> 40) & 0xff));
        this.writeByte((int)((x >>> 32) & 0xff));
        this.writeByte((int)((x >>> 24) & 0xff));
        this.writeByte((int)((x >>> 16) & 0xff));
     ...
     * @throws RuntimeException if any character in the string is not between 0 and 255.
     */
     ...
     * @throws RuntimeException if any character in the string is not between 0 and 2<sup>r</sup> - 1.
     */
