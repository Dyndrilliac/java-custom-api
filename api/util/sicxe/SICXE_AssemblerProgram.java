...
public class SICXE_AssemblerProgram extends SimpleSymbolTable
{
    ...
    protected static final SeparateChainingSymbolTable<String,SICXE_OpCode>
        constructDirectiveTable(final SeparateChainingSymbolTable<String,SICXE_OpCode> symTable)
    {
    ...
    protected static final SeparateChainingSymbolTable<String,SICXE_OpCode>
        constructInstructionTable(final SeparateChainingSymbolTable<String,SICXE_OpCode> symTable)
    {
    ...
    protected static final int evaluateExpression(final String expression, final SICXE_AssemblerProgram asmProgram)
    {
        ...
        if (expression != null)
        {
            ...
            else if (expression.matches(SICXE_Lexer.SICXE_IDENTIFIERS + SICXE_Lexer.SICXE_OPERATORS + Lexer.INTEGERS))
            {
                left = SICXE_AssemblerProgram.resolveSymbol(parts[0].trim(), asmProgram);
                right = SICXE_AssemblerProgram.resolveOperand(parts[1].trim(), asmProgram);
            }
            else if (expression.matches(Lexer.INTEGERS + SICXE_Lexer.SICXE_OPERATORS + SICXE_Lexer.SICXE_IDENTIFIERS))
            {
                left = SICXE_AssemblerProgram.resolveOperand(parts[0].trim(), asmProgram);
    ...
    protected void assembleProgram()
    {
        ...
        try
        {
            ...
            if (!this.isPass1Error())
            {
                ...
                {
                    StdOut.println("Errors (Pass 2): partial object code generation, but no object file instantiation. " + "Refer to " +
                        this.getFileName() +
                        SICXE_AssemblerProgram.fileExtLst +
                        ".\n");
                }
                else
                {
            ...
            {
                StdOut.println("Errors (Pass 1): no object code generated. " + "Refer to " +
                    this.getFileName() +
                    SICXE_AssemblerProgram.fileExtMid +
                    ".\n");
            }
        }
