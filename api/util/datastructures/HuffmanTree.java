...
/*
 * Although we were only concerned with characters 'A'-'G' for this assignment, I designed the program to be usable with other contiguous subsets of
 * the alphabet. The alphabetString field is where this is controlled. The string MUST be in ascending order by the integer value of each character
 * according to the ASCII table and there cannot be any gaps. I've built a method to do this automatically thus avoiding tedious human typo errors.
 * 
 * @formatter:off
 * 
 * ALPHABET_A5 is the 'A'-'G' alphabet specified by the instructions on Blackboard.
 * ALPHABET_ASCII allows the processing of spaces and all printable ASCII characters.
 * 
 * @formatter:on
 * 
