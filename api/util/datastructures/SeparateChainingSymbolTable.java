...
{
	// Largest primes <= 2^i for i = 5 to 31.
	...
	public final void delete(final K key)
	{
		...
		if ((this.getMaxSize() > SeparateChainingSymbolTable.PRIMES[0]) &&
			(this.getCurSize() <= (2 * this.getMaxSize())))
		{
	...
	protected final void setCapacity(final int capacity)
	{
		if ((capacity < 0) || (capacity >= SeparateChainingSymbolTable.PRIMES.length))
		{
			if (capacity < 0)
			{
				...
				if (capacity >= SeparateChainingSymbolTable.PRIMES.length)
				{
