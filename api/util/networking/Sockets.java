...
public final class Sockets
{
     ...
     * Specifically, this
     * class represents the portion of the server which listens for new connection attempts. Every time a SimpleAbstractServer object
     * accepts a new
     * client, it spawns a SimpleAbstractServerThread object to handle the client. This allows for concurrency and results in a more robust
     * server.
     */
    public static abstract class SimpleAbstractServer implements Runnable
    {
        ...
        public SimpleAbstractServer(final int listeningPort)
        {
            ...
            {
                this.print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ", Color.RED, "Could not bind to port " + this.getListeningPort() + ": " + e.getMessage() + "\n");
                Support.displayException(this.getWindow(), e, false);
            }
        ...
        protected int findClient(final String identifier)
        {
            ...
            {
                if ( identifier.equals(this.getClients().get(i).getIdentifier()) ) { return i; }
            }
        ...
        public synchronized void remove(final String identifier)
        {
            ...
            if ( pos >= 0 )
            {
                this.print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ", Color.BLUE, "Removing client " + identifier + " at " + pos + "...\n");
     ...
     * This object encapsulates part of the necessary networking functionality for a simple socket-based TCP/IP server application.
     * Specifically, this
     * class represents the portion of the server which services a connected user's requests. Every time a SimpleAbstractServer object
     * accepts a new
     * client, it spawns a SimpleAbstractServerThread object to handle the client. This allows for concurrency and results in a more robust
     * server.
     */
    public static abstract class SimpleAbstractServerThread extends Thread
    {
        ...
        public void close()
        {
            ...
            {
                this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ", Color.RED, "Error closing connection to client " + this.getIdentifier() + "\n");
                Support.displayException(this.getWindow(), e, false);
            }
            finally
            {
        ...
        public void open()
        {
            ...
            {
                this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ", Color.RED, "Error opening connection to client " + this.getIdentifier() + "\n");
                this.close();
                Support.displayException(this.getWindow(), e, false);
            }
        }
        ...
        public void run()
        {
            this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ", Color.GREEN, "Server thread " + this.getIdentifier() + " running...\n");
            while ( this.getSocket() != null )
            {
                ...
                {
                    this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ", Color.RED, "Error reading " + this.getIdentifier() + ": " + e.getMessage() + "\n");
        ...
        public void send(final String message)
        {
            ...
            {
                this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ", Color.RED, "Error sending " + this.getIdentifier() + ": " + e.getMessage() + "\n");
                this.getServer().remove(this.getIdentifier());
                this.close();
                Support.displayException(this.getWindow(), e, false);
            }
