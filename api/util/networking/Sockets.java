...
public final class Sockets
{
	 ...
	 * Every time a SimpleAbstractServer object accepts a new client, it spawns a SimpleAbstractServerThread object to handle the client.
	 * This allows for concurrency and results in a more robust server.
	 */
	public static abstract class SimpleAbstractServer implements Runnable
	{
		...
		public void run()
		{
			while (this.getThread() != null)
			{
				...
				{
					this.print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ",
						Color.BLUE, "Waiting for a client...\n");
					this.addThread(this.getListeningSocket().accept());
				}
				catch (final Exception e)
				{
					this.print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ",
						Color.RED, "Server accept error: " + e.getMessage() + "\n");
					this.stop();
					Support.displayException(this.getWindow(), e, false);
				}
			}
	...
	public static abstract class SimpleAbstractServerThread extends Thread
	{
		...
		public void close()
		{
			...
			{
				this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ",
					Color.RED, "Error closing connection to client " + this.getIdentifier() + "\n");
				Support.displayException(this.getWindow(), e, false);
			}
			finally
			{
		...
		public void open()
		{
			...
			{
				this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ",
					Color.RED, "Error opening connection to client " + this.getIdentifier() + "\n");
				this.close();
				Support.displayException(this.getWindow(), e, false);
			}
		}
		...
		{
			this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ",
				Color.GREEN, "Server thread " + this.getIdentifier() + " running...\n");
			while (this.getSocket() != null)
			{
				...
				{
					this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ",
						Color.RED, "Error reading " + this.getIdentifier() + ": " + e.getMessage() + "\n");
		...
		public void send(final String message)
		{
			...
			{
				this.getServer().print(Color.MAGENTA, "[" + Support.getDateTimeStamp() + "] ",
					Color.RED, "Error sending " + this.getIdentifier() + ": " + e.getMessage() + "\n");
				this.getServer().remove(this.getIdentifier());
				this.close();
				Support.displayException(this.getWindow(), e, false);
			}
